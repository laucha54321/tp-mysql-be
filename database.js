...
export async function createPersona(param) {
  const result = await pool.query(
    ...
    [
      param.nombre,
      param.apellido,
      await bcrypt.hash(param.contrasena, 11),
      param.email,
...
export async function createCurso(param) {
  const result = await pool.query(
    `
        ...
        VALUES (?,?)`,
    [param.nombre_curso, param.descripcion]
  );
  return result;
}
//#endregion
...
export async function createCursoPersona(param) {
  const result = await pool.query(
    `
        ...
        VALUES (?,?,?)`,
    [param.ID_Persona, param.ID_Curso, param.categoria]
  );
  return result;
}
...
export async function createCursoPersonaNota(param) {
  const result = await pool.query(
    `
        ...
        VALUES (?,?,?,?)`,
    [param.ID_Persona, param.ID_Curso, param.descripcion, param.nota]
  );
  return result;
}
